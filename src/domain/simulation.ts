...
// Per BUSINESS_LOGIC.md §3.4: additive linear interpolation from MIN..MAX
// over condition, plus a per-race form offset (constant across the race)
// plus a per-tick jitter perturbation. Drawing form/jitter is a separate
// concern (kept out so this stays pure).
export function computeSpeed(condition: number, form: number, jitter: number): number {
  const conditionRatio = condition / CONDITION_MAX
  const speedRange = BASE_SPEED_MPS_MAX - BASE_SPEED_MPS_MIN
  return BASE_SPEED_MPS_MIN + conditionRatio * speedRange + form + jitter
}
