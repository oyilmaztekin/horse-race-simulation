...
export function generateProgram(horses: Horse[], rng: Rng): Program {
  ...
  for (const distance of ROUND_DISTANCES) {
    ...
    const pool = horses.filter((h) => !previousLanes.has(h.number))
    const lanes: HorseId[] = []
    while (lanes.length < LANE_COUNT) {
      const totalWeight = pool.reduce((sum, h) => sum + h.condition, 0)
      ...
      const pickIdx = pool.findIndex((h) => (running += h.condition) > target)
