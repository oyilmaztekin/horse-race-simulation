...
- **Form scope: per (horse, race), drawn once at snapshot creation.** `drawForm(rng)` is called for each lane in lane-order 1→10 **before** the race begins. The result lives on `LanePosition.form` and is reused every tick — a horse that "has a good day" runs that day faster from start to finish. This is the dominant source of race-to-race upset variance among close-condition pairs (a 45 vs 55 race can flip across seeds; a 45 vs 80 race rarely does).
- **Jitter scope: independent per (horse, tick).** Every tick, every still-racing horse draws its own jitter sample from the shared RNG. The draw order within a tick is **lane 1 first → lane 10 last**, so a known seed reproduces an exact race. Jitter now exists for visual jiggle — outcome variance is owned by `form`.
- **RNG consumption order (frozen).** Per race: `LANE_COUNT` form draws in lane-order at snapshot creation, then per tick `LANE_COUNT` jitter draws in lane-order (skipping already-finished lanes). Same seed reproduces the same race byte-for-byte.
